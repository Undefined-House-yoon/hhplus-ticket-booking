...
export class QueueScheduler {
  ...
  async handleCron() {
    ...
    this.logger.debug(`Expired token IDs: ${expiredTokenIds}`);
    ...
    // const tokenListToBeActivated = await this.queueService.getTokenListToBeActivated(100);
    if (tokenListToBeActivated.length === 0) {
      return;
    }
    this.logger.debug(`tokenListToBeActivated : ${tokenListToBeActivated.length}`);
    const [activeTokenIds,activeSessionUserIds] = tokenListToBeActivated.map(({ id,userId }) => ([id,userId]));
    this.logger.debug(`Active token IDs: ${activeTokenIds} ${activeSessionUserIds.length}`);
