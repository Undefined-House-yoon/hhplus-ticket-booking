...
import {
  ...
  ticketStatus,
} from '../../../domain/auth/services/ticket-session.service';
...
describe('TicketSessionUseCase', () => {
  ...
  it('[should] 티켓 조회를 시작하면 티켓 세션을 추가합니다.', async () => {
    ...
    let session: TicketSession = service.findSessionByUserId(userId);
  ...
  it('[should fail] 이미 결제된 티켓으로 인해 상태가 변경되지 않습니다.', async () => {
    const userId = 1;
    await useCase.startViewing(userId);
    let session = service.findSessionByUserId(userId);
